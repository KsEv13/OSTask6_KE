...
//Quick Sort
int partition(int arr[], int low, int high, int bt[]) {
    int temp;
...
void quick_sort(int arr[], int low, int high, int bt[]) {
    if (low < high) {
        // partitioning the single array into two sub-arrays
        int pi = partition(arr, low, high, bt);
        // sorting the sub-arrays
        quick_sort(arr, low, pi - 1, bt);
        quick_sort(arr, pi + 1, high, bt);
    }
}
